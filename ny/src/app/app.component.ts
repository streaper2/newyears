import { Component, ViewChild  } from '@angular/core';
import { Nav, Platform , } from 'ionic-angular';
import { StatusBar, Splashscreen, SQLite } from 'ionic-native';

import { Page1 } from '../pages/page1/page1';
//import { Page2 } from '../pages/page2/page2';
import { ResolutionPage } from '../pages/resolution/resolution';
import { PastPage } from '../pages/past/past';
import { CaracterePage } from '../pages/caractere/caractere';
import { LoginPage } from '../pages/login/login';
import { ProfilPage } from '../pages/profil/profil';


import { AngularFire } from 'angularfire2';




@Component({
  templateUrl: 'app.html'
})
export class MyApp {
  @ViewChild(Nav) nav: Nav;



  rootPage: any = Page1;

  pages: Array<{title: string, component: any}>;

  constructor(public platform: Platform, public af: AngularFire) {
    this.initializeApp();
    Splashscreen.hide();

    af.auth.subscribe( user => {
      if (user) {
        this.rootPage = Page1;
      } else {
        this.rootPage = LoginPage;
      }
    });



    // used for an example of ngFor and navigation
    this.pages = [
      { title: 'Un peu d\'amour', component: Page1 },
      { title: 'Résolution', component: ResolutionPage },
      { title: 'Année 2016', component: PastPage },
      { title: 'Nos Caractère', component: CaracterePage },
      { title: 'Profil', component: ProfilPage }
    ];




  }




  initializeApp() {
    this.platform.ready().then(() => {
      // Okay, so the platform is ready and our plugins are available.
      // Here you can do any higher level native things you might need.
      StatusBar.styleDefault();
      Splashscreen.hide();
    });
  }

  openPage(page) {
    // Reset the content nav to have just this page
    // we wouldn't want the back button to show in this scenario
    this.nav.setRoot(page.component);
  }

  logout() {
    this.af.auth.logout();
  }
  
}
